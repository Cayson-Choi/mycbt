...
export default function AttemptDetailClient({
  ...
  const handleGradeSubmit = async () => {
    ...
    const subjectiveQuestions = result.questions.filter(
      (q) => q.question_type !== 'CHOICE' && q.grading_status !== 'AUTO'
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-12">
      <div className="max-w-5xl mx-auto px-4">
        ...
        {/* 수동 채점 버튼 */}
        {result.questions.some((q) => q.question_type !== 'CHOICE' && q.grading_status !== 'AUTO') && (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                ...
                                <span className="dark:text-gray-200">
                                  {choice}.{' '}
                                  <MathText text={question[`choice_${choice}` as keyof typeof question] as string} />
                                </span>
                              </div>
                            </div>
                          )
