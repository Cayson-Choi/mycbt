  ...
  const toggleCard = async (cardId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult({ type: 'success', message: `${CARD_LABELS[cardId]} ${newVisible ? '표시' : '숨김'}` })
      }
    } catch {
      setCards(prev => ({ ...prev, [cardId]: !newVisible }))
          ...
          >
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 focus:outline-none ${
                cards[cardId] ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-600'
              } ${saving === cardId ? 'opacity-50' : 'cursor-pointer'}`}
              role="switch"
