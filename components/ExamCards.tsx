...
export default function ExamCards({ initialExams }: { initialExams: Exam[] }) {
  ...
  useEffect(() => {
    ...
    const fetchExams = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // 카테고리 내 모든 시험을 플랫하게 변환
          const allExams: Exam[] = []
          for (const cat of categories) {
            for (const exam of cat.exams) {
              allExams.push({
                id: exam.id,
                name: `${cat.name}${exam.year ? ` ${exam.year}년 ${exam.round}회` : ""}`,
    ...
    const startPolling = () => {
      if (!interval) interval = setInterval(fetchExams, 10000)
    }
