...
## 최종 업데이트
2026-04-29 (v3 — 채용 연계 섹션 재디자인 + 랜딩 그래픽 정리)
...
- [x] 시험지 Word(.docx) 출력 — 로컬 dev 전용 (Pandoc 기반, 2단 레이아웃, KaTeX 수식 OMML 변환)
- [x] 시험지 인쇄 — 브라우저 인쇄 다이얼로그 (A4 2단, 좌우 여백 25mm)
- [x] 동영상 강의 관리(`/admin/videos`) — CRUD, 썸네일 URL/업로드 탭, 활성 토글, 등급 드롭다운
- [x] 시험 등급 인라인 변경 (`/admin/exams` 테이블에 select)
- [x] cbtbank.kr 외부 시험 일괄 임포트 (산업안전산업기사 14시험 1,400문제)

### 10. 동영상 강의 + 학습 메모
- [x] `Video` 모델 활용: 카테고리별 그룹(전기기사·전기기능사·승강기기능사)
- [x] 카테고리별 영상 모음 페이지(`/videos/{engineer|technician|elevator}`)
- [x] 사이트 안 풀스크린 모달 재생 (YouTube ID 자동 추출 + iframe 임베드)
- [x] 등급 기반 접근 제어 — 미로그인/등급부족 시 잠금 모달
- [x] `VideoMemo` 모델 + DB 연동 학습 메모 (800ms debounce auto-save)
- [x] `/my/memos` 메모 모음 페이지 — 카드 클릭 시 영상 모달 (메모 자동 로드)

### 11. 마이페이지 재디자인
- [x] 다크 프로필 히어로 (이니셜 아바타 + 등급 뱃지 + 가입일 + CAYSON 워터마크)
- [x] KPI 4카드 (총 응시 / 평균 / 최고 / 합격률) + 그라데이션 띠
- [x] 빠른 메뉴 4카드 (학습 메모 / 오답노트 / 프로필 / 홈) — SVG 아이콘
- [x] 시험별 성적 카드 + 평균 진행도 바
...
- [x] **CAYSON이 다른 이유** 6탭 그래픽 PNG 전환 (`public/tab-graphics/*.png`, next/image 최적화)
  - 이전 인라인 SVG/div 애니메이션 → 정적 PNG 6장(원본 대조/AI/즉시수정/CBT/약점/실시간)
- [x] **채용 연계 섹션 재디자인** (PremiumSection 하단)
  - 에디토리얼 헤더 + 핵심 지표 스트립(연계 기업 350+, 연 매칭 1,200+, 평균 입사 43일)
  - 4단계 매칭 타임라인 + 분야 분포 가로 막대 차트 (전기설비 90 / 건설 80 등 6분야)
  - 단일 앰버 액센트 + 다크 네이비, 픽토그램·외부 기업명 미사용
- [x] CTA 카드 다크 골드 프리미엄 디자인 (다크 네이비 배경 + 골드 텍스트/버튼)
  - 카드별 그레인(feTurbulence) + 골드 셰엔 보더 + 도트 콘스텔레이션 배경
- [x] 전체 여백 최적화 (모바일 + 데스크탑)
- [x] 모바일 글씨 크기 반응형 적용
- [x] 시험 화면 글씨/선택지 크기 축소 (긴 문제 대응)
- [x] 관리자 문제 관리 — 페이지 가로지르는 문제 일련번호 표시 (1페이지 1~20, 2페이지 21~40 …)
- [x] CountUp `toLocaleString()` 적용으로 1,200 같은 천 단위 콤마 표기
