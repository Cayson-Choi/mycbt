...
### 3단계: OCR 전 사전 스캔 (필수) ⭐
OCR 시작 전 반드시 아래 두 가지를 먼저 확인:
1. **시험 범위 파악**: 페이지 헤더를 스캔해서 연도·회차·과목명·과목당 문제 수를 정확히 파악. 절대 임의로 가정하지 말 것 (예: "100문제겠지" → 실제로는 80문제인 경우 있음)
2. **정답 위치 파악**: PDF마다 정답 표기 방식이 다름. 전체 문서를 훑어서 정답이 어디에 어떤 형태로 표시되는지 먼저 확인 (해설 마지막 박스, 별도 정답 페이지, 문제 옆 표기 등 다양)
...
- **수학 공식은 KaTeX 문법으로 변환:**
  ...
  - **보기에 수식이 있어도 반드시 KaTeX 적용** — 보기라고 텍스트로 넘어가지 말 것 (예: `$\dfrac{G_2(s)}{1-G_1 G_2 H}$`)
- **[그림]** 표시: 텍스트로 표현 불가능한 도표/회로도/그래프/표가 있으면 `[그림]` 삽입
  - **문제에 표(Table)가 있으면 무조건 [그림] 처리** (텍스트로 옮기지 말 것)
  - 회로도, 그래프, 벡터도, 파형도도 [그림]
  - 단순 나열(인화점/착화점 같은 리스트)만 텍스트로 옮겨도 됨
- **보기 번호 배치 규칙** (원본 레이아웃에 따라 결정):
  - 2×2 블록 배치: 좌상=①, 우상=②, 좌하=③, 우하=④ (행 우선)
  - 세로 1열 배치: 위에서부터 순서대로 ①②③④
- **원본 그대로 추출** — 콤마/마침표, 띄어쓰기, 단위 표기 등 임의 수정·정정 절대 금지. KaTeX 변환만 허용
- **정답불명 처리**: 정답을 확인할 수 없으면 `a: 0`으로 표시. 절대 추측 금지. OCR 완료 보고 시 정답불명 문제 번호를 반드시 명시 (수동 확인 필요)
  - DB에서 `answer=0`인 문제는 채점 대상에서 자동 제외 (점수 합산 안 됨)
  - 결과 화면에 "⚠️ 정답불명 — 관리자 확인 필요" 표시
...
2. **텍스트 부분 제거, 그림만 크롭** — 핵심!
   ...
   - **상하좌우 여백이 같아야 함** → 그림을 프레임 중앙에 두고 네 방향 여백을 동일하게 유지 (한 쪽만 붙거나 떨어지지 않도록 조정)
   - **글자가 짤려서 남아 있으면 안 됨** → 문제번호, 선지번호(①②③④), 질문 텍스트, 해설 텍스트 조각이 상단·하단·좌우 가장자리에 일부라도 보이면 다시 크롭. **순수하게 문제에 주어진 그림만** 남도록 해야 함
3. **Agent 비전 크롭 방법:**
